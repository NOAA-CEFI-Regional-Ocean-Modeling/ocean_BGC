...
public find_eta, thickness_to_dz
...
subroutine thickness_to_dz_3d(h, tv, dz, G, GV, US, halo_size)
  ...
  if ((.not.GV%Boussinesq)) then !.and. allocated(tv%SpV_avg))  then
    ...
    !enddo ; enddo ; enddo
    call MOM_error(FATAL, "thickness_to_dz not support fully non-Boussinesq mode")
  else
    do k=1,nz ; do j=js,je ; do i=is,ie
