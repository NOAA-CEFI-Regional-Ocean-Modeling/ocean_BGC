...
!         call generic_miniBLING_register(tracer_list)
  ...
  !  </IN>
  ! </SUBROUTINE>
  subroutine generic_tracer_init(isc,iec,jsc,jec,isd,ied,jsd,jed,nk,ntau,axes,grid_tmask,grid_kmt,grid_depth,init_time)
    ...
    real, dimension(:,:)         , intent(in) :: grid_depth
    type(g_tracer_type), pointer    :: g_tracer,g_tracer_next

    character(len=fm_string_len), parameter :: sub_name = 'generic_tracer_init'

    call g_tracer_set_common(isc,iec,jsc,jec,isd,ied,jsd,jed,nk,ntau,axes,grid_tmask,grid_kmt,grid_depth,init_time) 
