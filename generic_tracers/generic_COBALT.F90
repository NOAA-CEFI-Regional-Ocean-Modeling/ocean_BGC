    ...
    if (do_dms_diag) then
       ...
       do j = jsc, jec ; do i = isc, iec  !{
         ...
         cobalt%chl_dmsp(i,j) = max(min(cobalt%f_chl(i,j,1)*cobalt%Rho_0*1e-3, &
          ...
          cobalt%dmspos_mix(i, j) = ((10.0 ** log10dmsp_mix) / 1e6) * grid_tmask(i,j,1)     ! convert from nmol L to mol m-3
          ...
          cobalt%dmspos_strat(i, j) = ((10.0 ** log10dmsp_strat) / 1e6) * grid_tmask(i,j,1)     ! convert from nmol L to mol m-3
