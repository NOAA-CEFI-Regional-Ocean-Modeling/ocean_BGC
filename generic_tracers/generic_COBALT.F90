...
!            implementation='duce',   &
  ...
  subroutine generic_COBALT_update_from_bottom(tracer_list, dt, tau, model_time)
    ...
    if (do_CBED) then
      call generic_CBED_sediments_update_from_bottom(tracer_list, cobalt, phyto, dt, tau, model_time)
    else
      call generic_COBALT_sediments_update_from_bottom(tracer_list, cobalt, phyto, dt, tau, model_time)
    endif

  end subroutine generic_COBALT_update_from_bottom

  subroutine generic_COBALT_sediments_update_from_bottom(tracer_list, cobalt, phyto, dt, tau, model_time)
    type(g_tracer_type), pointer :: tracer_list
  ...
  !     ilb,jlb,tau,dt,grid_dat,model_time,nbands,max_wavelength_band,sw_pen_band,opacity_band,internal_heat,frunoff)
    ...
    allocate(rho_dzt_bot(isc:iec,jsc:jec)); rho_dzt_bot(:,:)=0.0
    allocate(k_bot(isc:iec,jsc:jec)); k_bot(:,:)=0
    !
...
!
    if (do_CBED) then
      ...
      call generic_COBALT_sediments_update_from_source(tracer_list, cobalt, phyto, ilb, jlb, mask_coast, &
           grid_tmask, grid_dat, grid_kmt, isc,iec, jsc,jec, isd, jsd, nk, r_dt, dt, frunoff, rho_dzt, dzt, internal_heat)
    endif !do_CBED
...
!==============================================================================================================
  ...
  subroutine generic_COBALT_sediments_update_from_source(tracer_list, cobalt, phyto, ilb, jlb, mask_coast, &
           grid_tmask, grid_dat, grid_kmt, isc,iec, jsc,jec, isd, jsd, nk, r_dt, dt, frunoff, rho_dzt, dzt, internal_heat)
